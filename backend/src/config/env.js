...
export const getEnv = () => {
  ...
  return {
    ...
    MONGODB_URI:
      process.env.MONGODB_URI ?? (isTest ? "" : (() => { throw new Error("MONGODB_URI is required"); })()),
    JWT_SECRET:
      (process.env.JWT_SECRET || process.env.JWT_ACCESS_SECRET) ?? (isTest ? "" : (() => { throw new Error("JWT_SECRET or JWT_ACCESS_SECRET is required"); })()),
