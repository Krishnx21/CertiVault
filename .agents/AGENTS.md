...
## Phase 2.5: L3 Verification Gate — MANDATORY Before Any Issue Creation or Assignment Request

> **Context:** ECSoC26 labeling is handled by an automated bot, NOT by the repository maintainers. The bot classifies the work based on the issue content and the PR diff. Once the bot assigns a level (e.g., L2), rebuttals and complaints to maintainers are ineffective — the bot's decision is final. Therefore, the agent MUST guarantee that every issue we create or request assignment on genuinely qualifies as L3 BEFORE taking action.

1. **L3 Qualification Checklist — HARD GATE:** Before creating a new issue OR commenting to request assignment on an existing issue, the agent MUST verify the issue passes ALL of the following L3 criteria. If ANY criterion fails, the issue is NOT L3 — do NOT claim it as L3.

    **An issue is L3 (Core/Architecture/Performance) ONLY if it involves:**
    - [ ] **Core backend logic changes** — modifying fundamental algorithms, data pipelines, database schemas, API route handlers, authentication/authorization flows, or core business logic.
    - [ ] **Architectural restructuring** — introducing new design patterns (DI, event-driven, CQRS), restructuring module boundaries, creating new abstraction layers, or refactoring tightly-coupled services into decoupled components.
    - [ ] **Performance optimization** — implementing caching layers, optimizing database queries/indexes, reducing algorithmic complexity (e.g., O(n²) → O(n log n)), adding connection pooling, or batch processing.
    - [ ] **Security hardening** — fixing IDOR, CSRF, SSRF, SQL injection, improper auth, secrets exposure, or implementing new security middleware.
    - [ ] **Concurrency/reliability** — fixing race conditions, implementing retry logic, adding circuit breakers, or solving deadlocks.

    **An issue is NOT L3 if it primarily involves:**
    - ✗ UI/UX changes (styling, layout, responsiveness, animations, color schemes, dark mode)
    - ✗ Adding buttons, modals, tooltips, or frontend components
    - ✗ Documentation, README, or CONTRIBUTING.md updates
    - ✗ Typo fixes, linting, or formatting changes
    - ✗ Adding frontend form validation (unless it's backend validation middleware)
    - ✗ Simple CRUD endpoint additions without architectural significance
    - ✗ Adding environment variable support or config file changes alone
    - ✗ Accessibility audits or translation tasks

2. **Creating New L3 Issues:** When creating a new issue intended to be L3, the issue title and body MUST clearly demonstrate core backend/architecture/performance work. Use concrete technical language that an automated classifier bot would recognize as L3. Examples of strong L3 signals in issue bodies:
...
3. **Requesting Assignment on Existing Issues:** Before commenting to request assignment on an existing issue, the agent MUST:
    ...
    - NEVER claim an issue is L3 in your comment if it is actually a UI fix, documentation task, or simple feature addition — the bot WILL downgrade it and we lose credibility.

## Phase 3: Before Starting Work on an Issue

1. **Comprehensive Review Before Action:** Before writing a single line of code, the agent MUST:
   ...
   - Check the acceptance criteria carefully — these are your definition of "done".
2. **Assignment Requests for L3/Core Features:** Before submitting an issue assignment request for core features or L3 tasks, you MUST provide a proper issue assignment template in the comment. This proposal must detail how you plan to tackle the issue, outline your step-by-step implementation plan, list the specific files to be modified/created, and cover any other relevant architectural details.
3. **Wait for Assignment:** **DO NOT start working on any issue until the Admin has officially assigned it to the user.** Premature work risks wasted effort if someone else is assigned.

## Phase 4: During Development

1. **No Scratch Files in Commits — MANDATORY PRE-STAGE AUDIT:** NEVER commit temporary, generated, or scratch files to the repository. This includes:
...
2. **Atomic, Meaningful Commits:** Every commit message MUST follow Conventional Commits format. Each commit should represent one logical change. Never use messages like "fix", "update", "wip", "changes", or "misc". Bad example: `Fix CI`. Good example: `fix(evaluation): remove unused numpy import to resolve ruff F401`.
3. **Understand Before Fixing:** When something fails (CI, tests, lint), ALWAYS read the FULL error log before attempting a fix. Do not guess. Specifically:
    ...
    - For lint/type errors: Read every reported error — there may be multiple distinct issues.
    - Fix ALL errors in a single commit when possible, not one-at-a-time.

## Phase 5: Before Pushing / Creating a PR

1. **Pre-Push Quality Gates:** Before pushing any code, the agent MUST run and pass ALL of these locally:
    - **Linting:** `ruff check nightmarenet/ tests/` — 0 errors
    - **Type checking:** `mypy nightmarenet/ --ignore-missing-imports` — 0 errors (use the exact flags the CI uses)
    - **Tests:** `pytest tests/` — all tests pass
...
2. **Merge Upstream Before PR:** Before creating or updating a PR, always:
    - `git fetch upstream`
    - `git merge upstream/main`
    - Resolve conflicts locally (never through the GitHub UI)
    - Re-run ALL quality gates after resolving conflicts
3. **Verify Clean Working Tree — HARD STOP BEFORE PUSH:** This is a non-negotiable gate. Before running `git push`:
    1. Run `git status` — confirm no untracked scratch/draft files exist in the repo root or any subdirectory.
    2. Run `git diff --staged` — read every single staged change. Verify that ONLY production code changes are included.
...
1. **Verify CI After Every Push:** After pushing code, ALWAYS check CI status:
...
2. **Never Claim Success Prematurely:** Do not say "the tests should pass" or "CI should be green now" unless you have concrete evidence. Either run the checks locally and show passing output, or wait for CI to actually complete and confirm.

## Phase 7: Communication & Professionalism

1. **Code of Conduct:** Ensure all communication (issue bodies, comments, PR descriptions) is highly professional, respectful, and adheres to the project's code of conduct. Write as a senior engineer — concise, technical, and courteous.
2. **Responding to Review Feedback:** 
    - Address EVERY point raised by the reviewer — never ignore or skip feedback.
...
- **Prevention Over Correction:** Because the bot's decision is essentially final, the primary strategy is **prevention** — ensuring every issue we create or work on is genuinely, unmistakably L3 before we ever touch it. This is why the L3 Verification Gate (Phase 2.5, Rules 9-11) exists and is mandatory.
- **Strong Rebuttals (Last Resort):** If the bot assigns an objectively incorrect label to a PR that demonstrably involves core architecture, performance optimization, or security hardening, you MAY drop a polite but firm rebuttal comment. The rebuttal MUST:
  ...
  4. Do NOT argue for L3 if the work was genuinely L2 — this damages credibility for future rebuttals.
